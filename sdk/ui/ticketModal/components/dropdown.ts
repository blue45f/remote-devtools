...
export function createCustomDropdown(config: CustomDropdownConfig): {
  ...
  confirmButton.addEventListener('mouseenter', () => {
    confirmButton.style.background = 'linear-gradient(135deg, #6d28d9, #4f46e5)';
  ...
  let tempSelectedValue: string | Set<string> = multiple ? new Set<string>() : '';
    ...
    insertionOrder.forEach((category) => {
      ...
      categoryOptions.forEach((option) => {
        ...
        if (multiple) {
          ...
          checkbox.checked = (tempSelectedValue as Set<string>).has(option.value);
  ...
  const getValue = () => {
    return multiple ? Array.from(selectedValue as Set<string>) : (selectedValue as string);
  };
