...
describe('resolveSeed', () => {
  it('returns dashboard stats for /api/dashboard/stats', () => {
    const result = resolveSeed<{ data: { totalTickets: number } }>('/api/dashboard/stats');
    expect(result?.data.totalTickets).toBeGreaterThan(0);
  });

  it('parses period from ticket trend query string', () => {
    const day = resolveSeed<{ data: unknown[] }>('/api/dashboard/tickets/trend?period=day');
    const month = resolveSeed<{ data: unknown[] }>('/api/dashboard/tickets/trend?period=month');
  ...
  it('returns live sessions for /sessions', () => {
    ...
    expect((result as { recordMode: boolean }[]).every((s) => !s.recordMode)).toBe(true);
  });
  ...
  it('wraps paginated record requests in {rows,nextCursor} and applies q filter', () => {
    ...
    expect(page.rows.every((r) => r.name.toLowerCase().includes(term))).toBe(true);
  });
  ...
  it('returns metadata for the session detail endpoint', () => {
    const meta = resolveSeed<{ id: number; url: string }>('/api/session-replay/sessions/1000');
  ...
  it('returns activity feed entries with required fields', () => {
    const feed =
      resolveSeed<{ id: string; kind: string; title: string; at: string }[]>('/api/activity/feed');
    expect(feed?.length).toBeGreaterThan(0);
    if (!feed) throw new Error('Expected seeded feed entries');
    for (const item of feed) {
      expect(item.id).toMatch(/^[a-z]+-\d+-\d+$/);
      expect(['session', 'ticket', 'error', 'join', 'comment']).toContain(item.kind);
