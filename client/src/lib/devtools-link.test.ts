...
describe('buildDevToolsLink', () => {
  ...
  it('includes recordMode=true and the recordId for recorded sessions', () => {
    const url = buildDevToolsLink('billing-modal-bug', 42, 'http://localhost:3000');
  ...
  it('upgrades to wss when the host is HTTPS', () => {
    const url = buildDevToolsLink('session', undefined, 'https://devtools.example.com');
    expect(url.startsWith('https://devtools.example.com/tabbed-debug/?wss=')).toBe(true);
  });
