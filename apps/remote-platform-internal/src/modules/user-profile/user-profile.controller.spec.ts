...
describe('UserProfileController', () => {
  ...
  describe('findOne', () => {
    ...
    it('should propagate NotFoundException', async () => {
      mockService.findOneByEmpNo.mockRejectedValue(new NotFoundException('User not found'));

      await expect(controller.findOne('00000000')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('remove', () => {
    ...
    it('should propagate NotFoundException for missing user', async () => {
      mockService.removeByEmpNo.mockRejectedValue(new NotFoundException('User not found'));

      await expect(controller.remove('00000000')).rejects.toThrow(NotFoundException);
    });
  });
});
