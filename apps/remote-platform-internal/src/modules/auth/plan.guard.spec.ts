...
function guardWith(
  ...
  return new PlanGuard(
    ...
    makeModuleRef(repository) as unknown as ConstructorParameters<
      typeof PlanGuard
    >[1],
  );
}
...
describe("PlanGuard", () => {
  ...
  it("prefers organization plan over token plan", async () => {
    ...
    await expect(
      guard.canActivate(
        makeCtx({ sub: "u1", org: "org-42", plan: "pro" } as AuthClaims),
      ),
  ...
  it("falls back to token plan when organization lookup fails", async () => {
    ...
    await expect(
      guard.canActivate(
        makeCtx({ sub: "u1", org: "org-42", plan: "starter" } as AuthClaims),
      ),
    ).resolves.toBe(true);
  });
  ...
  it("forbids callers with an unknown plan label", async () => {
    ...
    await expect(
      guard.canActivate(
        makeCtx({ sub: "u1", plan: "garbage" as unknown as OrganizationPlan }),
      ),
    ).rejects.toBeInstanceOf(ForbiddenException);
  });
  ...
  it("loads organization plan when repository returns it", async () => {
    ...
    await expect(
      guard.canActivate(
        makeCtx({ sub: "u1", org: "  org-42  ", plan: "free" } as AuthClaims),
      ),
    ).resolves.toBe(true);
