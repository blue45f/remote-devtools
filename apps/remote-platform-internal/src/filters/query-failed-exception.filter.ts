...
export class QueryFailedExceptionFilter implements ExceptionFilter {
    ...
    if (exception.message.includes('duplicate key value violates unique constraint')) {
      ...
      if (exception.message.includes('slack_id')) {
        const slackIdMatch = exception.message.match(/Key \(slack_id\)=\(([^)]+)\)/);
      ...
      // 사번 중복
        ...
        const empNoMatch = exception.message.match(/Key \(emp_no\)=\(([^)]+)\)/);
      ...
      // 디바이스 ID 중복
        ...
        const deviceMatch = exception.message.match(/Key \(device_id\)=\(([^)]+)\)/);
