...
describe('HealthController (External)', () => {
  ...
  it('check() should return healthy status when DB is up', async () => {
  ...
  it('check() should return error status when DB is down', async () => {
  ...
  it('ready() should report readiness via a DB ping', async () => {
    const healthy: HealthCheckResult = {
      status: 'ok',
      info: { database: { status: 'up' } },
      error: {},
  ...
  it('ready() should report not-ready when DB is down', async () => {
    const unhealthy: HealthCheckResult = {
      status: 'error',
      info: {},
      error: { database: { status: 'down', message: 'Connection refused' } },
