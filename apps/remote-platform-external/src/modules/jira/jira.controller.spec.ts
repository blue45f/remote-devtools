...
describe('JiraController', () => {
  ...
  describe('uploadImageToJira', () => {
    ...
    it('should throw BadRequestException when no file', async () => {
      await expect(controller.uploadImageToJira('PROJ-123', undefined as never)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw 500 on unknown service error', async () => {
      mockJiraService.uploadImageToJira.mockRejectedValue(new Error('Network timeout'));
