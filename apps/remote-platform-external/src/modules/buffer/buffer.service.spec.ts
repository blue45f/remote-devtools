...
describe('BufferService', () => {
  ...
  const makeEvent = (method: string, timestamp: number, params: unknown = {}): BufferEvent => ({
    method,
    params,
    timestamp,
  });
  ...
  describe('addEvent', () => {
    ...
    it('should evict oldest event when MAX_BUFFER_SIZE_PER_SESSION is exceeded', () => {
      ...
      const buffersMap = Reflect.get(service, 'buffers') as Map<string, SessionBuffer>;
      ...
      expect(sessionBuffers[0].events[sessionBuffers[0].events.length - 1]).toEqual(overflowEvent);
    });
  });
  ...
  describe('loadPreviousSessionData', () => {
    it('should return empty array when no previous data exists', () => {
      const result = service.loadPreviousSessionData('device-1', 'Session-room1', 1);
    ...
    it('should return copied events when data exists', () => {
      ...
      const result2 = service.loadPreviousSessionData('device-1', 'Session-room1', 1);
  ...
  describe('mergeSessionData', () => {
    ...
    it('should create a new buffer if none exists and merge previous events', () => {
      ...
      service.mergeSessionData('Session-new-room', 5, 'device-1', previousEvents);
