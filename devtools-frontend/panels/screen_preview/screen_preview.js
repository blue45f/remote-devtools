...
export class ScreenPreviewPanel extends UI.Widget.VBox {
  ...
  renderPreview({ isMobile, width, height, head, body, bodyClass, baseHref }) {
    ...
    if (iframeContent.head.innerHTML !== headHtml) {
      iframeContent.head.innerHTML = headHtml;
    }

    if (baseHref) {
      let baseElement = iframeContent.head.querySelector('base[data-screen-preview-base="true"]');
      if (!baseElement) {
        baseElement = iframeContent.createElement('base');
        baseElement.setAttribute('data-screen-preview-base', 'true');
        iframeContent.head.prepend(baseElement);
      }
      baseElement.href = baseHref;
    }

    iframeContent.body.innerHTML = body;
    iframeContent.body.setAttribute('class', bodyClass)
    // if (!isMobile) {
  ...
  rendeHtml() {
    const preview = ({ target }) => {
      if (this.startPreviewed) {
        this.startPreviewed = false;
        target.innerText = '화면 보기';
        this.onSend('ScreenPreview.stopPreview');
      } else {
        this.startPreviewed = true;
        target.innerText = '화면 보기 Stop';
    ...
    this.iframe.style.borderRadius = '10px';
    this.iframe.srcdoc = '<!DOCTYPE html><html lang="ko"><head></head><body></body></html>'
