...
describe('RecordService', () => {
  ...
  beforeEach(async () => {
    ...
    repository = module.get<Repository<RecordEntity>>(getRepositoryToken(RecordEntity));
  });

  describe('create', () => {
    it('should create and save a record', async () => {
      vi.spyOn(repository, 'create').mockReturnValue(mockRecord as RecordEntity);
      vi.spyOn(repository, 'save').mockResolvedValue(mockRecord as RecordEntity);
  ...
  describe('findAll', () => {
    it('should return all records', async () => {
      vi.spyOn(repository, 'find').mockResolvedValue([mockRecord as RecordEntity]);
  ...
  describe('findPreviousByDeviceId', () => {
    ...
    it('should query previous records by device and timestamp', async () => {
      ...
      expect(mockQb.where).toHaveBeenCalledWith('record.device_id = :deviceId', {
        deviceId: 'device-123',
      });
  ...
  describe('findPaginated', () => {
    ...
    it('returns rows + null cursor when fewer than limit results', async () => {
      const qb = makeQB([{ id: 1, timestamp: new Date('2026-04-27T10:00:00Z') }]);
