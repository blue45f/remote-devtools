...
describe('DomService', () => {
  ...
  beforeEach(async () => {
    ...
    domRepository = module.get<Repository<DomEntity>>(getRepositoryToken(DomEntity));
  ...
  describe('isGetDomResponseMessage', () => {
    it('should return true for DOM.GET_DOCUMENT id', () => {
      expect(service.isGetDomResponseMessage(MSG_ID.DOM.GET_DOCUMENT)).toBe(true);
    });
  ...
  describe('isGetDomRequestMessage', () => {
    it('should return true for DOM.getDocument method', () => {
      expect(service.isGetDomRequestMessage({ method: 'DOM.getDocument' })).toBe(true);
    });

    it('should return false for other methods', () => {
      expect(service.isGetDomRequestMessage({ method: 'Network.enable' })).toBe(false);
    });
  });
