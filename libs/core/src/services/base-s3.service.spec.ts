...
function createSampleData(overrides: Partial<BufferUploadData> = {}): BufferUploadData {
  return {
    room: 'test-room',
    recordId: 1,
    deviceId: 'device-123',
...
describe('BaseS3Service', () => {
  ...
  describe('buildCandidateDates', () => {
    it('should include dateHint when provided', () => {
      const result = service['buildCandidateDates'](1700000000000, '2023-11-14');
  ...
  describe('getBackupByDeviceAndTimestamp', () => {
    it('should return null for missing deviceId', async () => {
      const result = await service.getBackupByDeviceAndTimestamp('', 1700000000000);
    ...
    it('should return null for NaN timestamp', async () => {
      const result = await service.getBackupByDeviceAndTimestamp('device-123', NaN);

      expect(result).toBeNull();
    });

    it('should return null for Infinity timestamp', async () => {
      const result = await service.getBackupByDeviceAndTimestamp('device-123', Infinity);
    ...
    it('should return cached data when available in object cache', async () => {
      ...
      const cacheKey = service['buildObjectCacheKey']('device-123', 1700000000000);
      service['setCachedObject'](cacheKey, cachedData);

      const result = await service.getBackupByDeviceAndTimestamp('device-123', 1700000000000);
    ...
    it('should return a defensive clone from cache (not the same reference)', async () => {
      ...
      const result1 = await service.getBackupByDeviceAndTimestamp('device-123', 1700000000000);

      const result2 = await service.getBackupByDeviceAndTimestamp('device-123', 1700000000000);
