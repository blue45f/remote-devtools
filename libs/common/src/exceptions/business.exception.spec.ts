...
describe('BusinessException', () => {
  describe('constructor', () => {
    ...
    it('should use custom message when provided', () => {
      const ex = new BusinessException(ErrorCode.VALIDATION_FAILED, 'Custom error text');
  ...
  describe('static factory methods', () => {
    ...
    it('jiraError should return 503', () => {
      const ex = BusinessException.jiraError(ErrorCode.JIRA_CONNECTION_FAILED, 'Timeout');
      expect(ex.getStatus()).toBe(HttpStatus.SERVICE_UNAVAILABLE);
    });

    it('slackError should return 503', () => {
      const ex = BusinessException.slackError(ErrorCode.SLACK_CONNECTION_FAILED);
      expect(ex.getStatus()).toBe(HttpStatus.SERVICE_UNAVAILABLE);
    });
