...
function base64ToBytes(base64: string): Uint8Array {
  ...
  const bufferLength =
    (len * 3) / 4 - (base64[len - 1] === '=' ? 1 : 0) - (base64[len - 2] === '=' ? 1 : 0);
...
async function addImageToCanvas(imageUrl: string, sessionName: string) {
  try {
    ...
    if (imageUrl.startsWith('data:image')) {
      ...
      if (!base64Data) {
        throw new Error('잘못된 데이터 URL 형식입니다 (base64 데이터 없음)');
      }
      ...
      if (estimatedSizeInMB > 30) {
        throw new Error(
          `이미지 크기가 너무 큽니다: ${estimatedSizeInMB.toFixed(2)} MB (최대 30MB)`,
        );
      }
      ...
      try {
        ...
        throw new Error(
          `Base64 디코딩 실패: ${(decodeError && decodeError.message) || '알 수 없는 오류'}`,
        );
      }
    } else {
      ...
      if (!response.ok) {
        throw new Error(`이미지를 가져오지 못했습니다: ${response.status} ${response.statusText}`);
      }
    ...
    try {
      ...
      throw new Error(
        `이미지 생성 실패: ${(createError && createError.message) || 'Figma API 오류'}`,
      );
    }
    ...
    try {
      ...
      throw new Error(
        `이미지 크기 확인 실패: ${(sizeError && sizeError.message) || '크기 정보 없음'}`,
      );
    }
    ...
    console.log(
      `Final image size: ${finalWidth.toFixed(0)} x ${finalHeight.toFixed(0)} (${pageType})`,
    );
...
async function captureAndUploadToJira(issueId: string) {
  ...
  if (selection.length === 0) {
    ...
    setTimeout(() => {
      figma.notify(
        '💡 Tip: Shift+클릭으로 여러 요소를 선택하거나, 드래그로 영역을 선택할 수 있습니다',
        {
          timeout: 5000,
        },
      );
...
figma.ui.onmessage = (msg: PluginMessage) => {
  ...
  switch (msg.type) {
    ...
    case 'add-image': {
      ...
      if (imageUrl) {
        addImageToCanvas(imageUrl, msg.sessionName).catch((err) => {
          console.error('[addImageToCanvas] Unhandled error:', err);
          figma.notify('이미지 추가 중 오류가 발생했습니다', { timeout: 3000, error: true });
        });
      } else {
    ...
    case 'start-capture-mode': {
      ...
      if (currentSelection.length > 0) {
        ...
        figma.notify(`🎯 이미 ${currentSelection.length}개 요소가 선택되어 있습니다`, {
          timeout: 2000,
        });
        setTimeout(() => {
        ...
        setTimeout(() => {
          figma.notify('💡 Tip: 여러 요소를 선택하려면 Shift+클릭 또는 드래그를 사용하세요', {
            timeout: 5000,
          });
        }, 1000);
      }
