...
/**
 * 녹화 세션/티켓 클릭 이벤트 처리 - sessionName만 사용
 */
async function handleSessionClick(sessionName: string): Promise<void> {
  try {
    showMessage(`"${sessionName}" 이미지를 가져오는 중...`);
    ...
    // 녹화 세션 상세 정보 가져오기 (sessionName으로 조회)
    const sessionDetail = (await appState.apiClient.getSessionDetail(
      sessionName,
    )) as SessionDetailResponse;

    if (sessionDetail.screenPreviewUrl) {
      // screenPreviewUrl이 API 엔드포인트인 경우 ApiClient를 통해 dataURL 가져오기
      let imageDataUrl = sessionDetail.screenPreviewUrl;

      if (!sessionDetail.screenPreviewUrl.startsWith('data:')) {
        try {
          console.log('Screenshot URL:', sessionDetail.screenPreviewUrl);

          // URL에서 recordId 추출 (예: http://localhost:3001/sessions/generate-screenshot?recordId=123)
          const urlParams = new URLSearchParams(sessionDetail.screenPreviewUrl.split('?')[1]);
          ...
          if (recordId) {
            ...
            if (screenshotData.dataURL) {
              ...
              showMessage(`"${sessionName}"에 유효한 스크린샷이 없습니다`, 'error');
              return;
            }
          } else {
            console.error('No recordId found in URL:', sessionDetail.screenPreviewUrl);
      ...
      if (sessionDetail.recordId) {
        try {
          ...
          if (screenshotData.dataURL) {
            ...
            showMessage(`"${sessionName}"에 스크린샷이 없습니다`);
          }
        } catch (error) {
 ...
 */
export function initializeSessionHandlers(): void {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (sessionItem && !target.closest('a')) {
      // 링크 클릭은 제외
      event.preventDefault();
      const sessionName = sessionItem.dataset.sessionName || '';
      if (sessionName) {
        // 로딩 상태 시작
        sessionItem.classList.add('loading');

        try {
          await handleSessionClick(sessionName);
        } finally {
          // 로딩 상태 종료
...
function initDropZoneHandlers() {
  ...
  document.addEventListener('drop', async (event) => {
    ...
    try {
      ...
      dropZone.classList.remove('uploading');
      // Build the static structure, then set the (untrusted) error text via
      // textContent so an error message containing markup can't inject HTML.
      dropZone.innerHTML = `
        <div class="drop-zone-icon">❌</div>
        <p class="drop-zone-text">업로드 실패</p>
        <p class="drop-zone-hint"></p>
      `;
      const hintEl = dropZone.querySelector('.drop-zone-hint');
      if (hintEl) {
        hintEl.textContent = error instanceof Error ? error.message : '알 수 없는 오류';
      }
